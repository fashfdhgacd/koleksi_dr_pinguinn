...
function isJunkPutarinTitle(title: string): boolean {
  const t = cleanTitle(title || "");
  if (!t || t === "Video") return true;
  if (/^AI-\s*$/i.test(t)) return true;
...
export async function listCategory(slug: string, page = 1, limit = DEFAULT_PAGE_SIZE, _signal?: AbortSignal): Promise<PagedVideos> {
  ...
  if (key === "jav") {
    filtered = items.filter((x) => isPutarin(x) && !isJunkPutarinTitle(x.title));
  } else if (key === "ai-plus") {
    filtered = items.filter(isStreamtape);
  } else if (key === "videy") {
    filtered = items.filter(isVidey);
  } else {
