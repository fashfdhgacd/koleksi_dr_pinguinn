...
type RawItem = {
  ...
  source?: string;
  category?: string;
  /** YYYY-MM-DD dari bot / manual */
  date?: string;
  /** ISO timestamp saat di-update bot */
  updated_at?: string;
  tags?: string[];
};
...
/** Timestamp untuk sort terbaru dulu. updated_at > date > 0. */
function itemTime(it: RawItem): number {
  ...
  if (it.date) {
    // YYYY-MM-DD → anggap akhir hari agar tidak kalah dari item tanpa jam
...
/** Sort descending by time; item tanpa date tetap di belakang, urutan relatif dijaga. */
...
async function loadItems(): Promise<{
  ...
  // Bot feeds dulu (biasanya newest-first), baru local catalog
  const base: RawItem[] = [];
  ...
  // Local catalog & streamtape batch di belakang (sering tanpa date)
  ...
  // Dedup by id: simpan yang lebih baru (date/updated_at)
  ...
  for (const it of base) {
    ...
    if (!prev || isNewer(it, prev)) {
      ...
      // tetap merge field, tapi jangan turunkan date
...
export async function listLatest(page = 1, limit = DEFAULT_PAGE_SIZE, _signal?: AbortSignal): Promise<PagedVideos> {
  ...
  // mainCatalog sudah dari items yang di-sort terbaru; pastikan lagi sebelum page
...
/**
 * Featured / hero: pilih video berdasarkan time-slot 5 menit.
 * Semua client di slot jam yang sama mendapat set yang sama,
 * dan set berganti otomatis tiap 5 menit (selaras dengan refresh di use-catalog).
 * Default limit 8 agar Hero bisa auto-rotate di dalam list.
 */
export async function listFeatured(page = 1, limit = 8, _signal?: AbortSignal): Promise<PagedVideos> {
  const { items, posters, videyNo } = await loadItems();
  const main = mainCatalog(items);

  // Prioritas pool: IndoAV → UserBokep → seluruh main catalog
